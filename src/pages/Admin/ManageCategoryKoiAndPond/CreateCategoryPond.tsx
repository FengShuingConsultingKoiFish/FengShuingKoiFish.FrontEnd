...
interface CreateCategoryPondFormData {
  name: string
  description: string
}

const CreateCategoryPond: React.FC = () => {
  ...
  const {
    ...
    formState: { errors },
    reset
  } = useForm<CreateCategoryPondFormData>()
    ...
    try {
      ...
      if (response.data.isSuccess) {
        return response.data.result.some(
          (pondCategory: { name: string }) => pondCategory.name === name
        )
      }
      return false
    } catch (error) {
  ...
  const onSubmit: SubmitHandler<CreateCategoryPondFormData> = async (data) => {
    setIsLoading(true)
    ...
    try {
      const response = await axiosClient.post(
        "/api/Pond/Add-PondCategory",
        data
      )
      if (response.data.isSuccess) {
        toast.success("Thêm danh mục hồ thành công")
        reset()
        setTimeout(() => {
      ...
      console.error("Error creating Pond category:", error)
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-md">
        <h2 className="mb-4 text-2xl font-semibold">Tạo Danh Mục Hồ</h2>
...
export default CreateCategoryPond
