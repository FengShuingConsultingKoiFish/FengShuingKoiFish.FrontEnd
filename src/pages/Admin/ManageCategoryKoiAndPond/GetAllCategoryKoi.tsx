...
const GetAllCategoryKoi = () => {
  const navigate = useNavigate()
  const [categories, setCategories] = useState<KoiCategory[]>([])
  ...
  const handleEditDetail = (id: number) => {
    navigate(`/admin/quan-li-loai-ca-va-loai-ho/all-koi/edit/${id}`)
  }
    ...
    if (categoryToDelete !== null) {
      ...
      try {
        ...
        if (response.data.isSuccess) {
          ...
          toast.success("Xóa loại cá Koi thành công")
        } else {
          console.error(response.data.message)
        }
      } catch (error: unknown) {
        if (axios.isAxiosError(error) && error.response?.status === 400) {
          toast.error("Không thể xóa loại cá này vì đang được sử dụng.")
        } else {
          toast.error("Có lỗi xảy ra khi xóa loại cá.")
        }
  ...
  return (
    <div className="relative flex w-full flex-col">
      <h2 className="mb-4 text-2xl font-bold">Danh sách các loại cá Koi</h2>

      {isLoading ? (
        <p>Đang tải...</p>
      ) : categories.length === 0 ? (
        <p>Không có loại cá Koi nào</p>
      ) : (
      ...
      <ConfirmModal
        ...
        message="Bạn có chắc chắn muốn xóa loại cá Koi này?"
...
export default GetAllCategoryKoi
