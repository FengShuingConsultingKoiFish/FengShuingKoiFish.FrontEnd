...
interface EditCategoryKoiFormData {
  name: string
  description: string
}

const EditCategoryKoi: React.FC = () => {
  ...
  const {
    ...
    reset,
    formState: { errors }
  } = useForm<EditCategoryKoiFormData>()
  ...
  const onSubmit: SubmitHandler<EditCategoryKoiFormData> = async (data) => {
    setIsLoading(true)
    ...
    try {
      const response = await axiosClient.put(`/api/Koi/${id}`, data)
      if (response.data.isSuccess) {
        toast.success("Cập nhật danh mục cá Koi thành công.")
        navigate("/admin/quan-li-loai-ca-va-loai-ho/all-koi")
      } else {
...
export default EditCategoryKoi
