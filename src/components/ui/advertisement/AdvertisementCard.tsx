...
export const AdvertisementCard: React.FC<AdvertisementCardProps> = ({
  ...
  return (
    <div className="my-5 flex flex-col rounded-lg border bg-gray-200 shadow-2xl">
      <div className="p-4">
        <div className="flex flex-col space-y-3">
          <div className="flex justify-center items-center">
            <p className="text-2xl font-semibold">Quảng cáo</p>
          </div>
          <p className="font-semibold">Người đăng : {userName}</p>
        ...
        <div className="flex flex-wrap gap-1 p-4">
          {img.length === 1 ? (
            <img
              src={img[0]}
              alt="Post"
              className="h-full max-h-[1200px] w-full rounded-lg object-cover"
            />
          ) : (
      ...
      {activeBlogId === id && (
        <div className="mb-0 w-full p-4">
          {currentUser ? (
            <>
              {/* Comment Input Box */}
              <div className="mb-4 inline-flex w-full items-center justify-start gap-4 rounded-lg rounded-t-lg border border-gray-200 bg-transparent px-4 py-2">
                <textarea
