...
const checkAndRenewToken = async (dispatch: Dispatch): Promise<boolean> => {
  ...
  if (timeLeft <= renewThreshold) {
    ...
    console.log(
      `Token is about to expire, attempting to renew... Attempt ${renewalAttempts}`
    )
    ...
    try {
      ...
      if (
        renewalResponse.isSuccess &&
        renewalResponse.result?.token &&
        renewalResponse.result?.refreshToken
      ) {
